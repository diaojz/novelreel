...
当前是 **P0 文本闭环**。往后加「肉」的方向：

- **P1 · 生分镜图**：给每个分镜 / 每个角色生成配图（文生图）。
  关键是「角色一致性」——先为每个角色生成一张 sheet 图，后续所有分镜都参考它
  （这正是 ArcReel 的做法，也是前端那些「角色图」该来的地方，
  现在用的是占位图）。
- **P2 · 生视频 + 合成**：分镜图 → 视频片段（接 Seedance 等）→ ffmpeg 合成成片。

架构已经为这些留好了位置：模型客户端是独立的一层（换/加模型不动编排），
Subagent 是即插即用的（加一个 `generate_images.py` 就行），
编排器加一个状态判断就能把新步骤接进流水线。
